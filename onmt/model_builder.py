...
def load_rl_model(opt, dummy_opt, model_path=None):
    if model_path is None:
        if len(opt.models) > 1:
            prior_path = opt.models[0]
            agent_path = opt.models[1]
        else:
            prior_path = opt.models[0]
            agent_path = opt.models[0]

    prior_checkpoint = torch.load(prior_path,
                                  map_location=lambda storage, loc: storage)
    agent_checkpoint = torch.load(agent_path,
                                  map_location=lambda storage, loc: storage)

    fields = inputters.load_fields_from_vocab(
        agent_checkpoint['vocab'], data_type=opt.data_type)

    model_opt = agent_checkpoint['opt']
    ...
    prior = build_base_model(model_opt, fields, use_gpu(opt), prior_checkpoint)
    agent = build_base_model(model_opt, fields, use_gpu(opt), agent_checkpoint)
    prior.eval()
    agent.train()
    prior.generator.eval()
    agent.generator.train()
    return fields, prior, agent, model_opt, agent_checkpoint
